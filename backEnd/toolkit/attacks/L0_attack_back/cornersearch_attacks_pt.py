...
class CSattack(FixedEpsilonAttack):
    def __init__(
            self,
            model: Model,
            **args: Any):
    ...
    def run(
        ...
        **kwargs: Any
        ...
        corr_pred = get_predictions(model, x_nat, y_nat)
        ...
        for c in range(x_nat.shape[0]):
            if corr_pred[c]:
                ...
                for counter in range(self.n_corners):
                    ...
                    logit_2[counter * bs:(counter + 1) * bs] = get_logits(model,
                    ...
                    if not found:
                        for c2 in range(self.n_classes):
                            if not found:
                                ...
                                pred = get_predictions(model, batch_x, np.tile(batch_y, (batch_x.shape[0])))
        ...
        corr_pred = get_predictions(model, adv, y_nat)
