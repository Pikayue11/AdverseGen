...
class QueryModel():
    ...
    def __init__(self, defense_method='', model=None):
        if defense_method != '':
            ...
            self.defense_net = get_defense(defense_method, model)
        else:
            self.model = model
    ...
    def query(self, imgs, model, preprocess, labels):
        ...
        with torch.no_grad():
            if isinstance(model, Model):
                ...
                out = model(preprocess(imgs)).to(device)
            return self.get_query(out, labels)

    ## TODO: query adversarial label after successful attack
    def query_label(self, imgs, model, preprocess):
        # Query for no defense case
        with torch.no_grad():
            if isinstance(model, Model):
                out = model(imgs).to(device)
                ...
                out = model(preprocess(imgs)).to(device)
            return F.softmax(out, dim=1).argmax(dim=1)
...
def get_trans_advimg(imgs, model2, labels, raw_imgs, ba_num, max_distance=16.37):
...
class TrainModelS():
    ...
    def __call__(self, filenames, imgs, model2, labels, diff, query_score, query_loss, last_loss, optimizer,
        ...
        train_log_file = self.out_dir + '/train_log0'
        ...
        loss = nn.CrossEntropyLoss(reduction='none')(out,
                                                     c_labels)  # Note that using cross entropy loss to train surrogate model here
        ...
        loss2 = backward_loss + forward_loss * FL_rate
...
def before_query_iter(imgs, labels, model, model2, preprocess1, preprocess2, with_TIMI, with_s_prior,
                      max_distance=16.37):
    ...
    if correct.sum() > 0:
        ...
        if with_s_prior:
            best_advimg, adv_img = attack_black(imgs, labels, model, model2, preprocess1, preprocess2, counts, correct,
                                                last_query, max_distance=max_distance)
...
def run_attack_train(model, model2, data_loader, minibatch,
                     preprocess1, preprocess2, optimizer, log_name="", out_dir="", logger=None,
                     if_train=True, with_TIMI=True, with_s_prior=True, epsilon=0.1, max_distance=16.37, ba_interval=20,
                     ba_num=10):
    '''
    ...
    epsilon = epsilon  # epsilon for SimBA part
    max_distance = max_distance  # Max perturb budget  (L2 distance)
    b_num = 0
    # data_end=False
    get_new_flag = False

    query = QueryModel("", model)
    train_model_s = TrainModelS(out_dir)  # function for train surrogate model
    ...
    while True:
        ...
        if it % 50 == 1 or get_new_flag:  # Per 50 iteration, add new input data, and save success samples.
                ...
                with open(out_dir + '/' + log_name, 'a')  as f:
                    for i in range(len(imgs)):
                        if correct[i] == False or counts[i] > max_query:
                            ...
                            f.write(filenames[i] + ' Success:%d, adv_label:%d' % (~correct[i],
                                                                                  query.query_label(imgs, model,
                                                                                                    preprocess1)) + ' counts:%d, L2:%.5f, end_type:%d \n' % (
            ...
            if correct.sum() < minibatch:
                indices *= correct_all
                new_imgs, new_labels, new_filenames = get_data(data_iter,
                                                               minibatch - (correct).sum())  # Get new data to attack
                ...
                if get_new:
                    ...
                    new_imgs, counts0, last_score0, last_query0, last_loss0, correct0, prior_prob0, end_type0 = \
                        before_query_iter(torch.cat(new_imgs), new_labels, model, model2, preprocess1, preprocess2,
                                          with_TIMI, with_s_prior, max_distance=max_distance)
                ...
                last_loss = last_loss0 if b_num == 1 else torch.cat([last_loss[correct], last_loss0]) if get_new else \
                last_loss[correct]
                imgs = new_imgs if b_num == 1 else torch.cat([imgs[correct], new_imgs]) if get_new else imgs[correct]
                raw_imgs = new_raw_imgs if b_num == 1 else torch.cat([raw_imgs[correct], new_raw_imgs]) if get_new else \
                raw_imgs[correct]

                filenames = new_filenames if b_num == 1 else index_(filenames,
                                                                    correct) + new_filenames if get_new else index_(
                    filenames, correct)
                labels = new_labels if b_num == 1 else torch.cat([labels[correct], new_labels]) if get_new else labels[
                    correct]
                prior_prob = prior_prob0 if b_num == 1 else torch.cat(
                    [prior_prob[correct], prior_prob0]) if get_new else prior_prob[correct]
                counts = counts0 if b_num == 1 else torch.cat([counts[correct], counts0]).to(device) if get_new else \
                counts[correct]
                end_type = end_type0 if b_num == 1 else torch.cat([end_type[correct], end_type0]).to(
                    device) if get_new else end_type[correct]
                last_improve = last_improve0 if b_num == 1 else torch.cat([last_improve[correct], last_improve0]).to(
                    device) if get_new else last_improve[correct]
                correct = correct0 if b_num == 1 else torch.cat([correct[correct], correct0]).to(device) if get_new else \
                correct[correct]


        if it % ba_interval == (ba_interval - 1) and with_s_prior:
            # Run TIMI
            adv_imgs = get_trans_advimg(imgs[correct], model2, labels[correct], raw_imgs[correct], ba_num)
            score3, d_score3, loss3, c3 = query(adv_imgs, preprocess1, labels[correct])
            # Update prior_prob
            prior_prob[correct] = normalizer((adv_imgs - raw_imgs[correct]).abs().clamp(1e-6,
                                                                                        1e6))  # + torch.rand(imgs[correct].shape).to(device)*0.2
            update_index = (d_score3 < last_query[correct]) | (
                ~c3)  # | ((last_query[correct]==1.0) & (last_improve[correct]>=80))
        ...
        if not with_s_prior:
            prior_prob = torch.ones(imgs.shape).to(device)
        selects = select_points(mode='by_prob', probs=prior_prob,
                                select_num=1)  # Select point according to prior prob got by TIMI.
        k_size = int((max_distance * 25 / 16.38 + 1) // 2 * 2 + 1)
        diff, diff_kernel = get_diff_gauss(selects, imgs.shape, reference,
                                           k_size=k_size)  # Add gaussian noise on select pixel.

        c1 = correct.clone()
        adv_imgs = proj(imgs[correct], diff[correct], raw_imgs[correct])
        score1, d_score1, loss1, c1[correct] = query(adv_imgs, preprocess1,
                                                     labels[correct])  # Query model1 with +diff noise
    ...
    if if_train:  # Save train weight of surrogate model
        torch.save(model2.state_dict(), out_dir + '/snapshot/%s_final.pth' % model2)
    adv_img = imread(out_dir + "/images/img.png")
    return adv_img, counts_all, correct_all, end_type_all, L2_all
...
def parse_args():
    ...
    parser.add_argument('--gpu_id', default="", help='using gpu id', type=str)
