import numpy as np
import tensorflow as tf # click "install" provided by pycharm # wky
tf.compat.v1.disable_eager_execution()  #wky
...
def build_ssim_graph(batch_shape=(None, 112, 112, 3)):
    ...
    X_tf = tf.compat.v1.placeholder(tf.float32, shape=batch_shape, name='X')    # wky
    Y_tf = tf.compat.v1.placeholder(tf.float32, shape=batch_shape, name='Y')    # wky
    S_tf = structural_similarity_tf(X_tf, Y_tf)
    G_tf, = tf.gradients(S_tf, Y_tf)
    Ga_tf = tf.reduce_max(tf.abs(G_tf), axis=-1, keepdims=True) # wky
    config_tf = tf.compat.v1.ConfigProto()  # wky
    config_tf.gpu_options.allow_growth = True
    tf_sess = tf.compat.v1.Session(config=config_tf)    # wky
